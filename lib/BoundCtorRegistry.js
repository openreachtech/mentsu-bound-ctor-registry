...
export default class BoundCtorRegistry {
  ...
  static ensureCtorPool ({
    ...
    if (this.BoundCtorPool.has(weakMapKey)) {
      return this.BoundCtorPool.get(weakMapKey)
    }
  ...
  ensureBoundCtor ({
    ...
    if (CtorPool.has(this.BaseCtor)) {
      return CtorPool.get(this.BaseCtor)
    }
 ...
 * @template {WeakMap | CompositeKeyPoolType<K, *>} [V = WeakMap | CompositeKeyPoolType<K, *>] - Value type.
 */
